...
class GeneticOverride (geneticgame.GeneticGame):
    ...
    def log (self):
        if self.gen % self.parms['eval']['interval'] == 0:
            ...
            fn = "stats"
            for parm in [('carryover', 'C'), ('crossovers', 'X'), ('matches_per_round', 'G'), ('population', 'P'), ('mutations', 'M'), \
                    ('survivor_ratio', 'S'), ('villages', 'V')]:
                val = str (self.parms[parm[0]])
                if 'poisson' in str (self.parms.get (parm[0] + '_fn', '')):
                    val = 'p' + val
                if val.startswith('0.'):
                    val = val[2]
                fn += parm[1] + val
