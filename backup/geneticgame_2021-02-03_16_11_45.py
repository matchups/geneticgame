...
class GeneticGame:
  ...
  def __init__ (self, _game, _parms):
    ...
    if isinstance(self.parms['eval']['outputter'], str): # If specified as a name, it's a method on this object
        self.parms['eval']['outputter'] = getattr (self, self.parms['eval']['outputter'])
  ...
  def build_descendants (self, parent_genes):
      ...
      info = {'start': copy.deepcopy(new_genes)}
      for cross_point in random.sample(range(self.parms['chrome_length']), k=self.parms['crossovers_fn']()):
          ...
          info [f"X{cross_point}"] = copy.deepcopy(new_genes)
      for who in (0, 1):
          for change_point in random.sample(range(self.parms['chrome_length']), k=self.parms['mutations_fn']()):
              ...
              info [f"D{who} {change_point}"] = copy.deepcopy(new_genes)
      if self.gen == 25  and  random.randrange(3) == 1:
          print ('----')
          for key, genes in info.items():
              msg = f"{key:5} "
              for who in (0, 1):
                  msg += ' '
                  for gene in range (24):
                      msg += str (genes[who][gene])
              msg += '->' + getattr(self.game, "display") (genes[1])
              print (msg)
  ...
  def eval(self, whether):
    ...
    for cnum in range (top_count):
        ...
        for onum in range (len(opponents)):
            p1, p2 = player, opponents[onum]
            switch = (cnum + onum) % 2 == 1 # Half the time, the evolving chromosome will go second
            if switch:
                p1, p2 = p2, p1
            res = self.game.play (p1, p2, self.parms['game_parms'])
            ret = res['score']
            if isinstance (ret, tuple): # Get just our score and ignore opponent's
                game_score, opp_score = ret
            else:
                game_score = ret
                opp_score = -ret
            if switch:
                game_score = opp_score
            score += game_score
            if 'log' in res  and  eval_parms['log']:
                logs[f"{cnum}-{onum}"] = res['log']
        updfn (cnum, score)
  ...
  def final (self, result_parms):
    ...
    for seq in self.get_top(count):
        ...
        if result_parms ['score']  or  all:
            ...
            msg += ' ' * (scorelen - len(scorestr)) + scorestr + ' '
  ...
  def rrpair (self):
      ...
      for round in range (chrome_count - 1):
        ...
        for col in range (table_count):
            r = random.randrange(2) # randomize who goes first
            round_order [col*2] = table [col][r]
            round_order [col*2 + 1] = table [col][1 - r]
      ...
      for roundnum in range (self.parms ['matches_per_round']):
          ...
          for matchnum in range (0, chrome_count, 2):
              ...
              if isinstance (score, tuple):
                  s1, s2 = score # non-zero-sum game, so each player has their own score
              else:
                  s1 = score # zero-sum game, so player two's score is the inverse of player one's
                  s2 = -score
